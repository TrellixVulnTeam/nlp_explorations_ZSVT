...
# its index representation.
# TODO Load our model's tokenizer (siebert/sentiment-roberta-large-english)
# Hint: Go to
# https://huggingface.co/siebert/sentiment-roberta-large-english and click
# on "Use in Transformers" (top right)
# tokenizer = ...
...
def predict_sentiment_of_sentence(sentence):
    """
    Predicts sentiment of a given sentence.
    :param sentence:
    :return:
    """
    # Convert sentence_text into its index representation by calling the
    # tokenizer
    ...
    # output is a tensor (=vector or matrix) of size 1x2 because the model classifies
    # the given input for two classes, NEGATIVE and POSITIVE. It currently does not
...
def sentiment_classification(docs):
    ...
    each of the docs from the list of docs and some other texts.
    :param docs:
    :return:
    """
    ...
    # later but feel free to add more sentences to this list.
    my_sentences = [
        "Awesome.",
...
def pipeline():
    ...
    :return:
    """
    # Parse the Excel file (only get the first five rows)
    df = parse_dataset(5)
